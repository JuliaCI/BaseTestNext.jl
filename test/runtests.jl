...
tse_str = sprint(show, BaseTestNext.TestSetException(1,2,3))
@test contains(tse_str, "1 passed")
@test contains(tse_str, "2 failed")
@test contains(tse_str, "3 errored")

@test BaseTestNext.finish(BaseTestNext.FallbackTestSet()) != nothing
...
@test_throws ErrorException BaseTestNext.test_approx_eq(ones(10),ones(11),1e-8,"a","b")
@test_throws ErrorException BaseTestNext.test_approx_eq(ones(10),zeros(10),1e-8,"a","b")
...
@test typeof(ts) == BaseTestNext.DefaultTestSet
@test typeof(ts.results[1]) == BaseTestNext.Pass

tss = @testset "@testset/for should return an array of testsets: $i" for i in 1:3
    @test true
end
@test length(tss) == 3
@test typeof(tss[1]) == BaseTestNext.DefaultTestSet
@test typeof(tss[1].results[1]) == BaseTestNext.Pass
...
using BaseTestNext: AbstractTestSet, Result, Pass, Fail, Error
immutable CustomTestSet <: BaseTestNext.AbstractTestSet
